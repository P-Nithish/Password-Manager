...
def login() :
    ...
    try:
        ...
        with open(f"{username}_key.txt", 'r') as file1 :
...
def signup() :
    ...
    if not os.path.isfile(f"{username}_key.txt") :
...
def password_manager_window(username, K1, K2, K3) :
        ...
        def save_password():
            ...
            if user and pwd:  # #################################### DOUBT ##################################
                ...
                with open(f"{username}.txt", 'a') as file3:
                    ...
                    pwd = fsqr.four_square_encrypt(pwd, K2, K3)
    ...
    def delete():
        ...
        def delete_user():
            ...
            if os.path.isfile(f"{username}.txt") :
                ...
                with open(f"{username}.txt", 'r') as D:
                    details = D.readlines()
                ...
                if len(details) != 0:
                    ...
                    for line in details :
                        if line.split()[0] == delU :
                            ...
                            with open(f"{username}.txt", 'w') as update:
        ...
        button_delete_user = tk.Button(delete_window, text = "Delete User", command = delete_user, bg = "gray34", fg = "orangered2", font = ('Comic Sans MS', 10, 'bold'))
    ...
    def view() :
        ...
        try:
            ...
            with open(f"{username}.txt", 'r') as v:
            ...
            if len(contents) != 0:
                ...
                for line in contents:
                    ...
                    pawd = fsqr.four_square_decrypt(line[1].strip(), K2, K3)
    ...
    def generate_password():
        ...
        def generate():
            length = int(entry_length_gen.get())
        ...
        label_length_gen = tk.Label(generate_password_window, text = "Password Length ", bg = '#F0F0F0')
        entry_length_gen = tk.Entry(generate_password_window)
        button_generate = tk.Button(generate_password_window, text="Generate Password", command=generate, bg="#2196F3", fg = "orangered2")

        label_length_gen.pack(anchor="w")
        entry_length_gen.pack(anchor="w")
...
# Configuring labels of login window :
...
# Configuring entry boxes of login window :
...
# Configuring Buttons in login window :
...
# Positioning components of login window :
